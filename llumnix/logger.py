...
DEFAULT_LOGGING_CONFIG = {
    "formatters": {
        "llumnix": {
            "class": "llumnix.logging_utils.NewLineFormatter",
...
def _trace_calls(log_path, root_dir, frame, event, arg=None):
    if event in ['call', 'return']:
        ...
        if not filename.startswith(root_dir):
            # only log the functions in the llumnix root_dir
            return
...
def enable_trace_function_call(log_file_path: str,
    ...
    llumnix root directory.

    Note that this call is thread-level, any threads calling this function
    will have the trace enabled. Other threads will not be affected.
    """
    logger.warning(
        "LLUMNIX_TRACE_FUNCTION is enabled. It will record every"
        " function executed by Python. This will slow down the code. It "
        "is suggested to be used for debugging hang or crashes only.")
    logger.info("Trace frame log is saved to %s", log_file_path)
    if root_dir is None:
        # by default, this is the llumnix root directory
