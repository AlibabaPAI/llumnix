...
class MigrationCoordinator:
    ...
    def migrate_out_running_request(self,
        ...
        return self._migrate_out_multistage(migrate_in_ray_actor, migrate_out_request)

    def migrate_out_waiting_request(self,
                                    migrate_in_ray_actor: "ray.actor.ActorHandle",
                                    migrate_out_request: LlumnixRequest) -> "MigrationStatus":
    ...
    def _migrate_out_multistage(self,
                               migrate_in_ray_actor: "ray.actor.ActorHandle",
                               migrate_out_request: LlumnixRequest) -> "MigrationStatus":
        ...
        while stage_count < self.max_stages:
            stage_count += 1
            status = self._migrate_out_onestage(migrate_in_ray_actor, migrate_out_request)
            if MigrationStatus.is_finished(status):
                return status
        # exceed max stages
        return MigrationStatus.FINISHED_SRC_ABORTED

    def _migrate_out_onestage(self,
                              migrate_in_ray_actor: "ray.actor.ActorHandle",
                              migrate_out_request: LlumnixRequest) -> "MigrationStatus":
