...
class LlumnixClientBladeLLM(MultiProcessingLLMClient):
    ...
    async def drop_request(self, req_id: int) -> None:
        llumnix_id = self.entrypoint_id2llumnix_id.pop(req_id, None)
        if llumnix_id:
            logger.info("Drop request: {}.".format(llumnix_id))
            self.llumnix_id2entrypoint_id.pop(llumnix_id, None)
            self.request_streams.pop(llumnix_id, None)
            self.timestamps_streams.pop(req_id, None)
            await execute_actor_method_async_with_retries(
                self.manager.abort.remote, "Manager", "abort", str(llumnix_id)
            )
