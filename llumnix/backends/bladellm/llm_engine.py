...
from blade_llm.service.communications.protocol import GenerateStreamMessage
...
class AsyncBackQueueWrapper(APIWrapper):
    ...
    def _put_request_outputs_to_server(self, request_outputs: List[GenerateStreamMessage],
    ...
    async def send(self, dst_instance_id: str, msg: GenerateStreamMessage):
        req_id = msg.req_id
        ...
        self.put_queue_args_queue.put_nowait((msg, self.request_server_map[req_id]))
        if msg.resp.is_finished:
            logger.info("msg_producer finish_request {}".format(req_id))
    ...
    def drop_request(self, request_id: int) -> None:
        ...
        logger.debug("msg_producer drop_request {}".format(request_id))

    def add_request(self, request_id: int, server_info: ServerInfo) -> None:
        self.request_server_map[request_id] = server_info
        logger.debug("msg_producer add_request {} {}".format(request_id, server_info))

    def clear(self):
        self.request_server_map = {}
        logger.info("msg_producer reset")
...
class AsyncLLMEngineLlumnixMixin:
    ...
    async def start(self, loop: asyncio.AbstractEventLoop):
        ...
        self.msg_producer: AsyncBackQueueWrapper = AsyncBackQueueWrapper(self.placement_group,
                                                                          self.instance_id,
    ...
    # TODO(KuilongCui): As a barrier is always used when a request is determined to be migrated, a request
    ...
    async def _handle_dropped_request(self):
        if self._dropped_req.qsize() > 0:
            for _ in range(self._dropped_req.qsize()):
                ...
                self.msg_producer.drop_request(req_id)
    ...
    async def add_request(self, server_info: ServerInfo, server_request: ServerRequest):
        ...
        self.msg_producer.add_request(server_request.id, server_info)
        # pylint: disable=protected-access
    ...
    async def drop_request(self, req_id: int):
        ...
        await self._client.drop_request(int(req_id))
    ...
    # metrics
    def get_num_msg_producer_cached_request(self):
        return len(self.msg_producer.request_server_map)
...
class BackendBladeLLM(BackendInterface):
    def __init__(
        ...
        for index, ip_addr in enumerate(self.src_worker_ip_address):
            self.worker_infos.append(WorkerInfo(ip_address=ip_addr, instance_id=self.instance_id, worker_id=index))
    ...
    def free_src_request(self, backend_request: LlumnixRequest) -> None:
        self.engine.msg_producer.drop_request(backend_request.request_id)
