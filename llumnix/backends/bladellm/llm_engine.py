...
class WaitUpdatedRequestDetail:
    def __init__(self, request_id: int, num_in_token: int, num_output_token: int):
        self.request_id = request_id
        self.num_output_token = num_output_token
        self.num_in_token = num_in_token
...
class AsyncBackQueueWrapper(APIWrapper):
    def __init__(self, scheduler, placement_group, node_id, instance_id, output_queue_type) -> None:
    ...
    async def send(self, req_id, msg, reset=False):
        if req_id not in self.scheduler.migrating_out_request_last_stage:
...
class AsyncLLMEngineLlumnixMixin:
    ...
    def __init__(self,
                 ...
                 wait_update_event_requests: Dict[int, WaitUpdatedRequestDetail],
                 ) -> None:
        ...
        self._migration_event = asyncio.Event()

        self.wait_update_event_requests: Dict[int, WaitUpdatedRequestDetail] = wait_update_event_requests

    @property
    def instance_info(self) -> InstanceInfo:
        return self.scheduler.llumnix_metrics.to_instance_info()

    async def start(self, loop: asyncio.AbstractEventLoop):
        await super().start(loop)
        self._client = self.init_client_from_engine()
        self.trans_wrapper: AsyncBackQueueWrapper = AsyncBackQueueWrapper(
            self.scheduler, self.placement_group,self.node_id, self.instance_id, self.output_queue_type)
        self.scheduler.request_server_map = self.trans_wrapper.request_server_map
        self.scheduler.llumnix_metrics.engine_init_metrics(self)
    ...
    def _update_request_inference_type(self, resp_list: List[WorkerStepResponse]):
        ...
        for gen_group in request_groups:
            ...
            if request_group_id in self.scheduler.id2group:
                ...
                self.scheduler.id2group[request_group_id]._inference_type = \
                    RequestInferenceType.DECODE if num_out_token > 0 else RequestInferenceType.PREFILL

    def _wake_migrated_requests(self, resp_list, step_requests):
        request_groups = resp_list[0].generation_groups.generation_group
        for gen_group in request_groups:
            request_group_id = gen_group.request_group_id
            if request_group_id in self.wait_update_event_requests:
                num_out_token = gen_group.generations[0].num_out_token
                if num_out_token == self.wait_update_event_requests[request_group_id].num_output_token:
                    self.scheduler.remove_running_request(request_group_id)
                    self.wait_update_event_requests[request_group_id].notify()

    async def update_callback(self, resp_list, step_requests):
        self._wake_migrated_requests(resp_list, step_requests)
    ...
    async def _handle_dropped_request(self):
        for req_id in self._dropped_req:
            self.scheduler.id2group[req_id]._status = RequestStatus.FINISHED
    ...
    async def wake_engine(self):
        completed, pending = await asyncio.wait(
            [self._req_buffer.get(), self._stop_event.wait(), self._migration_event.wait()],
...
class AsyncLLMEngineLlumnix(AsyncLLMEngineLlumnixMixin, AsyncLLMEngine):
    def __init__(self,
        ...
        AsyncLLMEngineLlumnixMixin.__init__(self, instance_id, output_queue_type, migration_config,
                                            placement_group, node_id, src_worker_ip_address,
                                            wait_update_event_requests)
...
class BackendBladeLLM(BackendInterface):
    def __init__(
        ...
        self.wait_update_event_requests: Dict[int, WaitUpdatedRequestDetail] = {}
        engine_cls = self._get_engine_cls()
        self.engine = engine_cls(instance_id, output_queue_type, migration_config, placement_group,
                                node_id, self.src_worker_ip_address, self.wait_update_event_requests, engine_args)
    ...
    # In remove_running_request, the request is not deleted immediately. Instead, it waits for update_callback to
    # update the request-related information to ensure data synchronization between the scheduler and the worker.
    # Additionally, after add_migrating_out_request_last_stage, any tokens generated by the removed request on the
    # migration source instance will not be returned to the api_server.
    ...
    async def send_blocks(self, dst_ray_actor: ray.actor.ActorHandle, request_id: int,
        ...
        all_worker_request_states = await dst_ray_actor.execute_async_engine_method.remote(
            "_run_workers", "migrate_cache", request)

        if not has_more:
            request_state = all_worker_request_states[0]
            if request_state.num_output_token + request_state.num_in_token == \
                len(self.engine.scheduler.id2group[request_id].paged_reqs[0].token_ids):
                self.engine.scheduler.remove_running_request(request_id)
            else:
                wait_detail = WaitUpdatedRequestDetail(request_id, request_state.num_in_token,
                                                        request_state.num_output_token)
                self.wait_update_event_requests[request_id] = wait_detail
                logger.debug("waiting for migrated request {} update", wait_detail)
                await wait_detail.wait()
                del self.wait_update_event_requests[request_id]
    ...
    def commit_dst_request(self, backend_request: LlumnixRequest) -> None:
        ...
        self.engine._migration_event.set()
        logger.debug("commit dst request {}", backend_request.request_id)

    # metrics
    def get_all_request_ids(self) -> List[int]:
        return list(self.engine.scheduler.get_all_request_ids())

    def get_wait_update_request_ids(self) -> List[int]:
        return list(self.wait_update_event_requests.keys())
