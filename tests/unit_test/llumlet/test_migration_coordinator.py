...
async def test_migrate_out_onestage(ray_env):
    ...
    migrate_in_actor.pre_alloc_cache.remote.return_value = ray_remote_call.remote(dst_blocks)
...
async def test_migrate_out_running_request(_, ray_env):
    ...
    migrate_in_actor.pre_alloc_cache.remote.return_value = ray_remote_call.remote([1])
    coordinator._migrate_out_onestage.side_effect = [MigrationStatus.FINISHED]
    status = await coordinator._migrate_out_running_request(migrate_in_actor, migrate_out_request)
...
async def test_migrate_out_waiting_request():
    ...
    migrate_in_actor.pre_alloc_cache.remote.return_value = ray_remote_call.remote(dst_blocks)
    status = await coordinator._migrate_out_waiting_request(migrate_in_actor, migrate_out_request)
    assert status == MigrationStatus.FINISHED
