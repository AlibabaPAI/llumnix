...
class RayRpcMigrationBackend(MigrationBackendBase):
    ...
    def migrate_cache(self,
        ...
        for start_idx in range(0, tot_blocks, self.num_migration_buffer_blocks):
            ...
            if send_worker_data:
                rpc_numpy_cache, src_data = ray.get(ray_obj)
            else:
                rpc_numpy_cache = ray.get(ray_obj)

        self.do_recv(rpc_numpy_cache, recv_blocks)
        if src_data:
            self.worker_put_data_dst_callback(src_data)
...
def get_migration_backend(migration_config: MigrationConfig,
    ...
    if backend in ['nccl', 'gloo']:
        target_migration_backend = RayColMigrationBackend(migration_config,
                                                          cache_engine,
                                                          local_rank,
                                                          scheduling_strategy,
                                                          is_driver_worker,
