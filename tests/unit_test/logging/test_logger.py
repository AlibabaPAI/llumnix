...
# https://github.com/vllm-project/vllm/blob/cd8249903f189c5f06424e67dbc6512ca494a046/tests/test_logger.py
...
def test_default_llumnix_root_logger_configuration():
    ...
    logger = logging.getLogger("llumnix")
...
def test_descendent_loggers_depend_on_and_propagate_logs_to_root_logger():
    ...
    LLUMNIX_LOGGING_CONFIG_PATH (default: None) are not configured and default
    behavior is activated."""
    root_logger = logging.getLogger("llumnix")
    root_handler = root_logger.handlers[0]

    unique_name = f"llumnix.{uuid4()}"
    ...
    with NamedTemporaryFile(encoding="utf-8", mode="w") as logging_config_file:
        ...
        with patch("llumnix.logging.logger.LLUMNIX_LOGGING_CONFIG_PATH",
            ...
            expected_message_snippet = (
                ...
                "LLUMNIX_LOGGING_CONFIG_PATH was given.")
            assert expected_message_snippet in str(ex_info)

        # Remember! The root logger is assumed to have been configured as
        # though LLUMNIX_CONFIGURE_LOGGING=1 and LLUMNIX_LOGGING_CONFIG_PATH=None.
        root_logger = logging.getLogger("llumnix")
        other_logger_name = f"llumnix.test_logger.{uuid4()}"
