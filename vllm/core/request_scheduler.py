...
class RequestScheduler:
    def __init__(self, 
        ...
        self.high_priority_instance_ids = [0,1,2,3] if self.enable_migrate else []
        # self.high_priority_instance_ids = []
    ...
    def dispatch(self, session_id: str, priority_type: int) -> int:
        self.num_request += 1
        ...
        if self.dispatch_strategy == 'unbalanced':
            ...
            instance_id = self.dispatch_load(session_id, priority_type)
    ...
    def dispatch_load(self, session_id: str, priority_type: int) -> int:
        if session_id not in self.session_instance:
            ...
            if len(self.instance_info) < self.num_instance:
                ...
                if priority_type == 1:
                    minn_load = 10000
                    for instance_info in self.sorted_instance_infos:
                        load = get_instance_load(instance_info, self.load_control_strategy, key='dispatch', dispatch_strategy='priority')
                        if load < minn_load:
                            minn_load=load
                            instance_id = instance_info.instance_id

            # logger.info(f"{priority_type} dispatch to {instance_id}")
            self.session_instance[session_id] = instance_id
            self.num_first_turn_request += 1
        else:
            instance_id = self.session_instance[session_id]
    ...
    def need_migrate(self) -> List[Tuple[int, int]]:
        if not self.enable_load_control_prefill:
            ...
            # need_migrate_balanced, need_migrate_prefill_v2
