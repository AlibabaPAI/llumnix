...
def get_correct_order_output(length: int = 1000, contain_block: bool = False):
    ...
    while i < length - 1:
        ...
        yield GenerateStreamResponse(
            ...
            usage=TokenUsage(
                prompt_tokens=0, completion_tokens=i + tokens_len, total_tokens=i + tokens_len
            ),
        )
        i += tokens_len
    yield GenerateStreamResponse(
...
def test_correct_order_output(contain_block):
    ...
    for i in range(1, 100):
        ...
        for reqeust_output in get_correct_order_output(output_length, contain_block):
            res.extend(client.process_output_order(request_id, reqeust_output))
...
def test_out_of_order_output(contain_block,is_finished_flag_at_last_output):
    ...
    for i in range(1, 100):
        ...
        print(f'test_out_of_order_output {request_id}-{contain_block}-{is_finished_flag_at_last_output}')
        res = []
        for reqeust_output in get_out_of_order_output(length=output_length,contain_block=contain_block,is_finished_flag_at_last_output=True):
            res.extend(client.process_output_order(request_id, reqeust_output))
        print(res)
        check_processed_output(res, output_length)
