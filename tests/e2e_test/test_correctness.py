...
async def run_vllm_v1(model, tensor_parallel_size, enable_pd_disagg, enable_routine_migration):
    ip = get_ip_address()
...
config_schema = "engine, migration_backend, tensor_parallel_size, enable_routine_migration, enable_simulator," \
...
async def test_correctness(ray_env, shutdown_llumnix_service, check_log_exception, model,
                           engine, migration_backend, tensor_parallel_size, enable_routine_migration, enable_simulator,
    ...
    if engine == "vLLM":
        ...
        engine_prompt_output = await run_vllm_v1(model, tensor_parallel_size, enable_pd_disagg, enable_routine_migration)
    ...
    if launch_mode == "local":
        if enable_pd_disagg:
            ...
            launch_commands.append(generate_launch_command_func(result_filename=str(prefill_port)+".out",
                                                    ...
                                                    instance_type="prefill",
                                                    enable_routine_migration=enable_routine_migration,
                                                    tensor_parallel_size=tensor_parallel_size))
