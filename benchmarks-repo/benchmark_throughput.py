...
async def async_request_gen(generator, qps: float, distribution="uniform", coefficient_variation: float = 0.0, priority_ratio = 0):
    while True:
        try:
            item = next(generator)
            if priority_ratio > 0:
...
def plot_latency_cdf(req_latencies, prefill_latencies, decode_latencies, results_filename, prefill_SLO, priority_type=0):
    fig_filename = os.path.splitext(results_filename)[0] + f"_latency_{priority_type}.png"
...
def plot_latency_slo(serving_latencies, inference_latencies, results_filename, priority_type=0):
    slos = [serving_latencies[i] / inference_latencies[i] for i in range(len(serving_latencies))]

    fig_filename = os.path.splitext(results_filename)[0] + f"_slo_{priority_type}.png"
...
async def benchmark(
    ...
    query_model1 = m1.measure(query_model)
    ...
    async_prompts = async_request_gen(
        iter(prompts), qps=qps, distribution=distribution, coefficient_variation=coefficient_variation, priority_ratio=priority_ratio)
    ...
    save_all_latencies_npy(m._all_latencies + m1._all_latencies, results_filename)
    if m1._latencies:
    ...
    return throughput, \
           (m._prefill_token_latencies, m1._prefill_token_latencies), \
           (m._decode_token_latencies, m1._decode_token_latencies), \
           (m._inference_latencies, m1._inference_latencies), \
           avg_instance_num, \
           (m._latencies, m1._latencies), \
           (m._request_ids, m1._request_ids), \
           (m._decode_latencies, m1._decode_latencies), \
           (m._request_lens, m1._request_lens)

def gen_random_response_lens(distribution: str, len_mean, len_range, num_prompts):
    print(f"dist info:{distribution}, {len_mean}, {len_range}")
    if distribution == 'uniform':
        if len_range == 0:
...
def sample_burstgpt_request(
    ...
    prompt_lens = [request_tokens[idx] if request_tokens[idx] <= 6144 else 6144 for idx in sampled_ids]
    response_lens = [response_tokens[idx] if response_tokens[idx] <= 6144 else 6144 for idx in sampled_ids]
...
def main():
    ...
    parser.add_argument('--variable_prompt_lens_distribution', choices=[
                        "uniform", "exponential", "capped_exponential", "zipf","sharegpt","burstgpt"], default="uniform")
    ...
    parser.add_argument('--prompts_filename', type=str)
    parser.add_argument('--gen_random_prompts', action='store_true')

    # group = parser.add_mutually_exclusive_group(required=True)
    parser.add_argument('--allow_variable_generation_length',
                       action='store_true')
    parser.add_argument('--fixed_max_tokens', type=int)
    ...
    parser.add_argument('--enable_priority', action='store_true')
    ...
    if args.prompts_filename:
        ...
        elif args.variable_response_lens_distribution=="burstgpt":
            prompts, prompt_lens, response_lens= sample_burstgpt_request(args.prompts_filename, args.random_prompt_count ,tokenizer)
    ...
    elif not args.prompts_filename:
