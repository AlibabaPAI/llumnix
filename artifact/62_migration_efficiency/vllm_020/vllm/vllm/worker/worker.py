...
class Worker:
    ...
    # def send_gpu_cache_ray(self,dst_rank, block_list):
    #     with torch.cuda.stream(self.migration_stream):
    #         num_blocks = len(block_list)
    #         dummy_key_cpu = self.cache_engine.dummy_key_cpu[:num_blocks]
    #         dummy_value_cpu = self.cache_engine.dummy_value_cpu[:num_blocks]
    #         with torch.cuda.stream(self.migration_stream):
    #             for i in range(self.cache_engine.num_layers):
    #                 for idx,block_num in enumerate(block_list):
    ...
    # def recv_gpu_cache_ray(self,src_rank, block_list):
    #     with torch.cuda.stream(self.migration_stream):
    #         num_blocks = len(block_list)
    #         dummy_key = self.cache_engine.dummy_key_cpu[:num_blocks]
    #         dummy_value = self.cache_engine.dummy_value_cpu[:num_blocks]
    #         for i in range(self.cache_engine.num_layers):
    ...
    def init_distributed_environment(self) -> int:
        ...
        # col.init_collective_group(world_size=2, rank=self.rank, backend="gloo")
        return self.rank
    ...
    def init_cache_engine(self, cache_config: CacheConfig, do_migrate_warmup=True) -> None:
        ...
        if self.parallel_config.max_replicas > 1 and do_migrate_warmup: #do warm up
            ...
            #     self.cache_engine.recv_gpu_cache((self.rank - 1 + tot_tanks)%tot_tanks,[0])
            # if self.rank % 2:
            #     self.recv_gpu_cache_ray((self.rank - 1 + tot_tanks)%tot_tanks,[0])
            #     self.send_gpu_cache_ray((self.rank + 1)%tot_tanks,[0])
            # else:
            #     self.send_gpu_cache_ray((self.rank + 1)%tot_tanks,[0])
            #     self.recv_gpu_cache_ray((self.rank - 1 + tot_tanks)%tot_tanks,[0])
