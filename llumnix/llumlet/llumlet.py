...
from llumnix.utils import MigrationType, RequestIDType, BackendType, is_traced_request
...
class Llumlet:
    ...
    async def generate(self, request_id: RequestIDType, server_info: ServerInfo, expected_steps: int, *args, **kwargs) -> None:
        if is_traced_request(server_info):
            set_timestamp(server_info, 'llumlet_generate_timestamp', time.perf_counter())
