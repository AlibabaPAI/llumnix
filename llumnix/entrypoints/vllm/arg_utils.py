...
from llumnix.backends.vllm.utils import check_engine_args, verify_instance_args
from llumnix.arg_utils import EntrypointsArgs, ManagerArgs, InstanceArgs, LlumnixArgumentParser
from llumnix.entrypoints.utils import LaunchMode
from llumnix.utils import get_engine_args_from_ray_internal_kv
...
def get_args(cfg, launch_mode: LaunchMode, parser: LlumnixArgumentParser, cli_args: "Namespace") \
    ...
    if manager_args.load_engine_args_from_ray:
        for instance_type in ['prefill', 'decode']:
            engine_args_registered = get_engine_args_from_ray_internal_kv(instance_type)
            verify_instance_args(instance_args, engine_args_registered)
        return entrypoints_args, manager_args, instance_args, engine_args

    check_engine_args(engine_args)
    verify_instance_args(instance_args, engine_args)
