...
from blade_llm.service.worker import worker_main, _WorkerProcesses
from blade_llm.service.utils import kill_parent_process

from llumnix.logging.logger import init_logger
from llumnix.utils import get_ip_address, update_environment_variables
from llumnix.constants import RAY_GET_TIMEOUT
...
class WorkerProcessActor:
    ...
    def init_worker_process(self, rank: int, serving_args: ServingArgs, args, kwargs) -> None:
        ...
        logger.debug("Init worker process, rank: {}, node_rank: {}".format(
            self.rank, serving_args.dist_inference_options.node_rank))
...
class _WorkerProcessesRay(_WorkerProcesses):
    ...
    def _worker_watch_dog(self):
        while self._running:
            ...
            for rank, worker in enumerate(self.workers):
                try:
                    has_dead = not ray.get(worker.is_alive.remote(), timeout=RAY_GET_TIMEOUT)
    ...
    def _exit_server(self):
        for rank, worker in enumerate(self.workers):
            try:
                if ray.get(worker.is_alive.remote(), timeout=RAY_GET_TIMEOUT):
                    logger.critical("Kill alive worker {} (pid {}, node_id: {}, gpu_ids: {}).".format(
                        rank, self.worker_pids[rank],
                        self.worker_node_and_gpu_ids[rank][0], self.worker_node_and_gpu_ids[rank][1]))
                    ray.get(worker.kill_worker_process.remote(), timeout=RAY_GET_TIMEOUT)
            except: # pylint: disable=bare-except
                pass
            ray.kill(worker)
        ...
        if not self._args.serving_multi_processing_options.disable_frontend_multiprocessing:
            kill_parent_process()
    ...
    def _spawn_workers_ray(self, serving_args: ServingArgs, args, kwargs):
        ...
        ray.get([worker.init_worker_process.remote(rank, serving_args, args, kwargs)
                for rank, worker in enumerate(self.workers)], timeout=RAY_GET_TIMEOUT)
...
def launch_worker_ray(placement_group: PlacementGroup, serving_args: ServingArgs, *args, **kwargs):
    return _WorkerProcessesRay(placement_group, serving_args, *args, **kwargs)
