...
class CacheEngine:
    ...
    def __init__(
        ...
        self.migration_cache_size = self.num_gpu_blocks
    ...
    def allocate_gpu_cache(self) -> List[KVCache]:
        ...
        # self.dummy_key_gpu = torch.empty(
        #         size=(self.num_gpu_blocks, *key_block_shape),
        #         dtype=self.dtype,
        #         device="cuda",
        #     )
        # self.dummy_value_gpu = torch.empty(
        #         size=(self.num_gpu_blocks, *value_block_shape),
        #         dtype=self.dtype,
        #         device="cuda",
        #     )
    ...
    def allocate_cpu_cache(self) -> List[KVCache]:
        ...
        self.dummy_key_cpu =  torch.empty(
                size=(self.migration_cache_size, *key_block_shape),
                dtype=self.dtype,
                pin_memory=pin_memory,
            )
        self.dummy_value_cpu = torch.empty(
                size=(self.migration_cache_size, *value_block_shape),
                dtype=self.dtype,
                pin_memory=pin_memory,
            )
        return cpu_cache
    ...
    def recv_gpu_cache_batch(self, src_rank, recv_blocks):
        ...
        torch.cuda.synchronize()
    def send_gpu_cache_v0(self, dst_rank, send_blocks):
        with torch.cuda.stream(self.cache_stream):
            group = None
    ...
    def recv_gpu_cache_v0(self, src_rank, recv_blocks):
        with torch.cuda.stream(self.cache_stream):
            group = None
            if self.parallel_config.migrate_backend == "gloo":
                group = get_instance_parallel_group()
    ...
    def send_gpu_cache(self, dst_rank, send_blocks):
        with torch.cuda.stream(self.cache_stream):
            ...
            # inds_value.add_(block_tensor_value)
            for i in range(self.num_layers):
                for idx,block_num in enumerate(send_blocks):
                    dummy_key_cpu[idx].copy_(self.gpu_cache[i][0][block_num])
                    dummy_value_cpu[idx].copy_(self.gpu_cache[i][1][block_num])
                # torch.gather(self.gpu_cache[i][0], 0, inds_key, out=dummy_key)
                # torch.gather(self.gpu_cache[i][1], 0, inds_value, out=dummy_value)
                # dummy_key_cpu.copy_(dummy_key)
                # dummy_value_cpu.copy_(dummy_value)
                
                if self.parallel_config.migrate_backend == "nccl":
                    torch.distributed.send(dummy_key, dst_rank, group=group)
                    torch.distributed.send(dummy_value, dst_rank, group=group)
                else:
                    torch.distributed.send(dummy_key_cpu, dst_rank, group=group)
                    torch.distributed.send(dummy_value_cpu, dst_rank, group=group)
        torch.cuda.Stream.synchronize(self.cache_stream)
    ...
    def recv_gpu_cache(self, src_rank, recv_blocks):
        with torch.cuda.stream(self.cache_stream):
            ...
            #         pin_memory=True)
            # else:
            dummy_key = self.dummy_key_cpu[:len(recv_blocks)]
            dummy_value = self.dummy_value_cpu[:len(recv_blocks)]
            # inds_key = torch.zeros_like(dummy_key, dtype=torch.int64, device="cuda")
            # inds_value = torch.zeros_like(dummy_value, dtype=torch.int64, device="cuda")
            # block_tensor_key = torch.tensor(
            #     recv_blocks, dtype=torch.int64, device="cuda").view(-1, 1, 1, 1, 1)
    ...
    def send_gpu_cache_block(self, dst_rank, send_blocks):
        with torch.cuda.stream(self.cache_stream):
            group = None
            if self.parallel_config.migrate_backend == "gloo":
                group = get_instance_parallel_group()
            for i in range(self.num_layers):
                for block_num in send_blocks:
                    if self.parallel_config.migrate_backend == "nccl":
                        torch.distributed.send(self.gpu_cache[i][0][block_num], dst_rank, group=group)
                        torch.distributed.send(self.gpu_cache[i][1][block_num], dst_rank, group=group)
                    else:
                        torch.distributed.send(self.gpu_cache[i][0][block_num].cpu(), dst_rank, group=group)
                        torch.distributed.send(self.gpu_cache[i][1][block_num].cpu(), dst_rank, group=group)
        torch.cuda.Stream.synchronize(self.cache_stream)

    def recv_gpu_cache_block(self, src_rank, recv_blocks):
        with torch.cuda.stream(self.cache_stream):
            key_block_shape = self.get_key_block_shape()
            value_block_shape = self.get_value_block_shape()
            dummy_key = torch.empty(
                size=(1, *key_block_shape),
                dtype=self.dtype,
                device="cuda" if self.parallel_config.migrate_backend == "nccl" else "cpu")
            dummy_value = torch.empty(
                size=(1, *value_block_shape),
                dtype=self.dtype,
                device="cuda" if self.parallel_config.migrate_backend == "nccl" else "cpu")
            group = None
            if self.parallel_config.migrate_backend == "gloo":
            ...
            for i in range(self.num_layers):
                for block_num in recv_blocks:
                    if self.parallel_config.migrate_backend == "nccl":
                        ...
                        torch.distributed.recv(dummy_key, src_rank, group=group)
                        torch.distributed.recv(dummy_value, src_rank, group=group)
                    self.gpu_cache[i][0][block_num] = dummy_key.cuda()
                    self.gpu_cache[i][1][block_num] = dummy_value.cuda()
        torch.cuda.Stream.synchronize(self.cache_stream)
    @staticmethod
