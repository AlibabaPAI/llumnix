...
class AsyncLLMEngineLlumnixMixin:
    ...
    async def start(self, loop: asyncio.AbstractEventLoop):
        ...
        self.trans_wrapper: AsyncBackQueueWrapper = AsyncBackQueueWrapper(
            self._scheduler, self.placement_group,self.node_id, self.instance_id, self.output_queue_type)
    ...
    def inject_request_barriers(self):
        async def finish_callback(resp_list, request_barriers: List[RequestBarrier], running_filter_request_ids: List[int]):
            try:
                logger.debug("barrier callback, wake {}", [request_barrier.request_id for request_barrier in request_barriers])
                ...
                logger.exception("barrier callback error")
