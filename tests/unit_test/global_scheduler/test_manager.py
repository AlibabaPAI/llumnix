...
def test_init_instances(ray_env, manager):
    ...
    _, instances = ray.get(manager.init_instances.remote(QueueType("rayqueue"), InstanceArgs(), engine_args,
                                                         node_id))
...
def test_load_registered_service(ray_env, manager, load_registered_service, enable_pd_disagg):
    ...
    for instance_type in instance_type_list:
        ...
        if load_registered_service:
            assert get_engine_args.unwrap_engine_args_if_needed().model == instance_type
        else:
            assert get_engine_args.unwrap_engine_args_if_needed().model == 'no_constraints'
    if load_registered_service:
