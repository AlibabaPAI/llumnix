...
def test_gen_next_engine_args_baldellm():
    ...
    next_engine_args = llumnix_engine_args_factory.gen_next_engine_args(
        BackendType.BLADELLM, engine_args,next_instance_agrs
    )
...
def test_gen_next_engine_args_baldellm_from_registered_service():
    ...
    engine_args = None
    next_engine_args = llumnix_engine_args_factory.gen_next_engine_args(
        BackendType.BLADELLM, engine_args, InstanceArgs(instance_type=InstanceType.PREFILL)
    )
    assert next_engine_args is not engine_args
    assert asdict(next_engine_args.load_engine_args()) == asdict(
    ...
    next_engine_args = llumnix_engine_args_factory.gen_next_engine_args(
        BackendType.BLADELLM, engine_args, InstanceArgs(instance_type=InstanceType.DECODE)
    )
    assert next_engine_args is not engine_args
    assert asdict(next_engine_args.load_engine_args()) == asdict(
        mocked_load_engine_args(
...
def test_gen_next_engine_args_baldellm_enable_port_increment():
    ...
    engine_args = BladeLLMEngineArgs(serving_args)
    instance_args = InstanceArgs(instance_type=InstanceType.PREFILL, semi_pd_prefill_server_port=10)
    next_engine_args = llumnix_engine_args_factory.gen_next_engine_args(
        BackendType.BLADELLM, engine_args, instance_args
    )
    assert next_engine_args.revised_args.semi_pd_prefill_server_port == instance_args.semi_pd_prefill_server_port
    assert next_engine_args is not engine_args
    assert (
