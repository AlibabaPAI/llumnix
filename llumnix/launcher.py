...
from llumnix.utils import (initialize_placement_group,
                           ...
                           get_engine_args_from_ray_internal_kv)
...
class Launcher:
    def __init__(self,
        ...
        if self.pdd_config.load_engine_args_from_ray:

            self.pd_engine_args = {}
            for instance_type in ['prefill', 'decode']:
                self.pd_engine_args[instance_type] = get_engine_args_from_ray_internal_kv(instance_type)
    ...
    def _get_next_engine_args(self, engine_args, instance_type: str):
        if not self.pdd_config.load_engine_args_from_ray:
            return engine_args

        new_engine_args = self.pd_engine_args[instance_type]
