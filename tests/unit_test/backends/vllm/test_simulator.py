...
async def test_backend(ray_env):
    ...
    engine_args = EngineArgs(model=try_convert_to_local_path("facebook/opt-125m"), download_dir="/mnt/model", worker_use_ray=True,
                             enforce_eager=True, disable_async_output_proc=True)
    instance_args = InstanceArgs(enable_routine_migration=True, enable_pre_stop_migration=False, request_migration_policy="SR",
                                 migration_backend="gloo", migration_buffer_blocks=16, migration_num_layers=1,
                                 migration_last_stage_max_blocks=4, migration_max_stages=5, migration_backend_init_timeout=20)
