...
class WorkerRequestSyncGroup:
    def __init__(self, request_group: Dict):
        ...
        # Backup migrated out request worker meta to avoid concurrent access to state_manager.request_group
        self._backup_request_groups = {}
        # The worker meta of a migrated-in request is first added to _new_request_groups to prevent concurrent
        # access to state_manager.request_group, and will be added to state_manager in the next handle_rpc_call
    ...
    def add_new_request_group(self, request_group_id, request_group_data):
        with self.lock:
    ...
    def backup_request_group(self, request_group_ids):
        with self.lock:
            for request_group_id in request_group_ids:
                self._backup_request_groups[request_group_id] = self.request_group[request_group_id]

    def remove_backup_request_group(self, request_group_ids):
        with self.lock:
            for request_group_id in request_group_ids:
                self._backup_request_groups.pop(request_group_id, None)

    def get_request_meta(self, request_group_id):
        with self.lock:
            assert request_group_id in self._backup_request_groups
            return self._backup_request_groups[request_group_id]
...
class GrpcMigrationBackend(MigrationBackendBase):
    ...
    def migrate_cache(self, src_handle, src_blocks: List[int], dst_blocks: List[int]) -> None:
        ...
        with grpc.insecure_channel(ip_address, options=self.channel_options) as channel:
            ...
            if src_handle.is_last_stage:
                ...
                worker_meta = pickle.loads(response.state_manager_meta)
                self.request_sync_group.add_new_request_group(response.request_id, worker_meta)
    ...
    def do_send(self, request, context):
        ...
        if request.is_last_stage:
            assert request.request_id in self.request_sync_group._backup_request_groups
            request_worker_meta = self.request_sync_group.get_request_meta(request.request_id)
            responce.state_manager_meta = pickle.dumps(request_worker_meta)
            logger.debug("Pickle state manager meta for request id {}, data length: {}".format(
                request.request_id, len(responce.state_manager_meta)))
...
def get_migration_backend(instance_id: int, worker_id: int, rank: int, migration_config: MigrationConfig,
                          request_sync_group: WorkerRequestSyncGroup, state_manager: StateManagerBase,
